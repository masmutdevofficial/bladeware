              ...
              <img
                :src="
                  produkData.product_image
                    ? 'https://backendtest.bladewaretech.com/uploads/products/' +
                      produkData.product_image
...
const fetchFinanceBoost = async () => {
  try {
    ...
    const res = await axios.get(
      "https://backendtest.bladewaretech.com/api/finance-boost",
      {
        headers: {
...
const submitProduk = async () => {
  ...
  try {
    ...
    const res = await axios.post(
      "https://backendtest.bladewaretech.com/api/submit-produk",
      {
        id_products: id,
        price,
        profit,
...
const toggleProduk = async () => {
  if (!showProduk.value) {
    try {
      const token = localStorage.getItem("jwt_token");
      const res = await axios.get(
        "https://backendtest.bladewaretech.com/api/get-produk",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
...
const fetchProfileData = async () => {
  try {
    ...
    const { data } = await axios.get(
      "https://backendtest.bladewaretech.com/api/get-data-boost",
    ...
    if (data.status === "success") {
      ...
      if (fromApiUrl) {
        ...
        avatarUrl.value = `https://backendtest.bladewaretech.com/storage/${clean.replace(/^storage\//, "")}`;
      } else {
